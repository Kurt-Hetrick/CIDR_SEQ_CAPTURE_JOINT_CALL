...
START_FILTER_SNV=`date '+%s'`
	...
	CMD=$CMD' --variant '$CORE_PATH'/'$PROJECT_MS'/MULTI_SAMPLE/'$PREFIX'.raw.HC.SNP.vcf.gz'
	...
	CMD=$CMD' -o '$CORE_PATH'/'$PROJECT_MS'/MULTI_SAMPLE/'$PREFIX'.raw.HC.HardFiltered.SNP.vcf.gz'
...
ls $CORE_PATH/$PROJECT_MS/MULTI_SAMPLE/$PREFIX".raw.HC.HardFiltered.SNP.vcf.gz.tbi"
