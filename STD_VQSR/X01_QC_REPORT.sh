...
SAMPLE_SHEET=$1
SAMTOOLS_DIR=$2 # needs to be a newer version to read a cram file. e.g. 1.6
DATAMASH_DIR=$3
...
>| $CORE_PATH/$PROJECT_MS/REPORTS/$QC_REPORT_MS/"QC_REPORT.MULTI."$TIMESTAMP".csv"

##################################################################
##### USE A FOR LOOP FOR EVERY UNIQUE SM_TAG IN SAMPLE SHEET #####
####### CREATE AN ARRAY FOR UNIQ SM_TAG/PROJECT_SAMPLE COMBOS ###########
...
CREATE_SAMPLE_ARRAY ()
{
	SAMPLE_ARRAY=(`awk 1 $SAMPLE_SHEET \
		...
		#  1  PROJECT_SAMPLE=the Seq Proj folder name
...
for SM_TAG in $(awk 'BEGIN {FS=","} NR>1 {print $8}' $SAMPLE_SHEET | sort | uniq );
	do
		CREATE_SAMPLE_ARRAY
			...
			>| $CORE_PATH/$PROJECT_MS/TEMP/$SM_TAG".QC_REPORT_TEMP_MS.txt"
		...
		# IF FILE EXISTS THE GRAB THE FIELDS I WANT, IF NOT THEN PRINT "NaN"	

		if [ -f $CORE_PATH/$PROJECT_SAMPLE/REPORTS/CONCORDANCE/$SM_TAG"_concordance.csv" ];
			then
				awk 1 $CORE_PATH/$PROJECT_SAMPLE/REPORTS/CONCORDANCE/$SM_TAG"_concordance.csv" \
		...
		# NEED TO ADD IN WHEN A FILE DOES NOT EXIT
		...
		# IF FILE DOES NOT EXIST PRINT "NaN", otherwise grab what I want from input
		...
		awk 'BEGIN {OFS="\t"} END {if ($2!="") {print $2,$6} \
			...
			$CORE_PATH/$PROJECT_SAMPLE/REPORTS/TI_TV/$SM_TAG"_All_.titv.txt" \
			| $DATAMASH_DIR/datamash transpose \
		>> $CORE_PATH/$PROJECT_MS/TEMP/$SM_TAG".QC_REPORT_TEMP_MS.txt"
		...
		awk 'BEGIN {OFS="\t"} END {if ($2!="") {print $2,$6} \
			...
			$CORE_PATH/$PROJECT_SAMPLE/REPORTS/TI_TV/$SM_TAG"_Known_.titv.txt" \
			| $DATAMASH_DIR/datamash transpose \
		>> $CORE_PATH/$PROJECT_MS/TEMP/$SM_TAG".QC_REPORT_TEMP_MS.txt"
		...
		awk 'BEGIN {OFS="\t"} END {if ($2!="") {print $2,$6} \
			...
			$CORE_PATH/$PROJECT_SAMPLE/REPORTS/TI_TV/$SM_TAG"_Novel_.titv.txt" \
			| $DATAMASH_DIR/datamash transpose \
		>> $CORE_PATH/$PROJECT_MS/TEMP/$SM_TAG".QC_REPORT_TEMP_MS.txt"
		...
		zgrep -v "^#" $CORE_PATH/$PROJECT_SAMPLE/MIXED/RELEASE/FILTERED_ON_TARGET/$SM_TAG"_MS_OnBait_MIXED.vcf.gz" \
			| awk '{MIXED_COUNT++NR} {DBSNP_COUNT+=($3~"rs")} \
				END {if (MIXED_COUNT!="") print MIXED_COUNT,(DBSNP_COUNT/MIXED_COUNT)*100 ; \
				else print "0","NaN"}' \
			| sed 's/ /\t/g' \
		...
		# tranpose from multiple rows to one row

		cat $CORE_PATH/$PROJECT_MS/TEMP/$SM_TAG".QC_REPORT_TEMP_MS.txt" \
		| $DATAMASH_DIR/datamash transpose \
		>| $CORE_PATH/$PROJECT_MS/REPORTS/QC_REPORT_PREP_MS/$SM_TAG".QC_REPORT_PREP.txt"

		#############################################################
		#### APPEND TO THE QC REPORT GENERATED AT THE BEGINNING #####
		#############################################################

		sed 's/\t/,/g' $CORE_PATH/$PROJECT_SAMPLE/REPORTS/QC_REPORT_PREP/$SM_TAG".QC_REPORT_PREP.txt" \
		>> $PROJECT_MS/$PROJECT_MS"-QC_REPORT.MS."$TIMESTAMP".csv"
