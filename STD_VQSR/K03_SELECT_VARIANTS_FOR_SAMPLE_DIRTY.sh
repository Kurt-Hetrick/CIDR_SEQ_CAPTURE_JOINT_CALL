...
SAMPLE_COLUMN=(`egrep -m 1 ^#CHROM $CORE_PATH/$PROJECT_MS/MULTI_SAMPLE/$PREFIX".BEDsuperset.VQSR.1KG.ExAC3.REFINED.vcf" \
	| sed 's/\t/\n/g' \
	| cat -n \
	| grep -w $SM_TAG \
	| sed 's/^ *//g' \
...
# Extract out sample, remove non-passing, non-variant
	...
	$CORE_PATH/$PROJECT_MS/MULTI_SAMPLE/$PREFIX".BEDsuperset.VQSR.1KG.ExAC3.REFINED.vcf" \
