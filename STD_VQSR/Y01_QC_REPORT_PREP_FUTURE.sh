...
##############################################################
##### GRABBING CONCORDANCE IF TRUTH SET IS FINAL REPORT. #####
##############################################################
##########################################################################
##### THIS IS THE HEADER #################################################
...
echo
	...
	GRAB_CONCORDANCE_FINAL_REPORT ()
	{
		...
		export -f GRAB_CONCORDANCE_FINAL_REPORT
...
##########################################################################
##### GRABBING CONCORDANCE SUMMARY METRICS IF TRUTH SET IS VCF FILE. #####
##########################################################################
##### THIS IS THE HEADER ###############################################################
##### "TRUTH_SAMPLE","HET_SENSITIVITY","HET_PPV","HOMVAR_SENSITIVITY","HOMVAR_PPV" #####
##### "VAR_SENSITIVITY","VAR_PPV","VAR_SPECIFICITY","GENOTYPE_CONCORDANCE" #############
##### "NON_REF_GENOTYPE_CONCORDANCE" ###################################################
########################################################################################

	GRAB_CONCORDANCE_VCF_SUMMARY_METRICS ()
	{
		PROJECT_SAMPLE=$1
		SM_TAG=$2
		CORE_PATH=$3
		...
		if [ -f ${CORE_PATH}/${PROJECT_SAMPLE}/REPORTS/CONCORDANCE_METRICS/RELEASE/${SM_TAG}genotype_concordance_summary_metrics ];
		then
			awk 'BEGIN {FS="\t";OFS="\t"} \
				NR=8 \
				{print $2,$4,$5,$7,$8,$10,$11,$12,$13,$14}' \
			| ${DATAMASH_DIR}/datamash transpose \
			>> ${CORE_PATH}/${PROJECT_MS}/TEMP/QC_REPORT_PREP_${PREFIX}/${SM_TAG}".QC_REPORT_TEMP.txt"
		else
			echo -e "NaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN\tNaN" \
		...
		export -f GRAB_CONCORDANCE_VCF_SUMMARY_METRICS
...
##############################################################################
##### GRABBING CONCORDANCE CONTINGENCY METRICS IF TRUTH SET IS VCF FILE. #####
##############################################################################
##### THIS IS THE HEADER #####################################################
##### "TP_COUNT","TN_COUNT","FP_COUNT","FN_COUNT","EMPTY_COUNT" ##############
##############################################################################

	GRAB_CONCORDANCE_VCF_CONTIGENCY_METRICS ()
	{
		PROJECT_SAMPLE=$1
		SM_TAG=$2
		CORE_PATH=$3
		...
		if [ -f ${CORE_PATH}/${PROJECT_SAMPLE}/REPORTS/CONCORDANCE_METRICS/RELEASE/${SM_TAG}genotype_concordance_contingency_metrics ];
		then
			awk 'BEGIN {FS="\t";OFS="\t"} \
				NR=8 \
				{print $4,$5,$6,$7,$8}' \
		...
		export -f GRAB_CONCORDANCE_VCF_CONTIGENCY_METRICS
