...
# For each PROJECT_MS in the sample sheet make a qc report containing only those samples in sample sheet.
...
# for SM_TAG in $(awk 'BEGIN {FS=","} $1=="'$PROJECT_MS'" {print $8}' $SAMPLE_SHEET | sort | uniq );
# 	do
# 		CREATE_SAMPLE_ARRAY

# 		cat $CORE_PATH/$PROJECT_MS/REPORTS/QC_REPORT_PREP/$SM_TAG".QC_REPORT_PREP.txt" \
# 		>> $CORE_PATH/$PROJECT_MS/TEMP/$SAMPLE_SHEET_NAME".QC_REPORT."$TIMESTAMP".txt"
# 	done
...
# ( cat $CORE_PATH/$PROJECT_MS/REPORTS/ANEUPLOIDY_CHECK/*.chrom_count_report.txt | grep "^SM_TAG" | uniq ; \
# cat $CORE_PATH/$PROJECT_MS/REPORTS/ANEUPLOIDY_CHECK/*.chrom_count_report.txt | grep -v "SM_TAG" ) \
# | sed 's/\t/,/g' \
# >| $CORE_PATH/$PROJECT_MS/REPORTS/QC_REPORTS/$PROJECT_MS".ANEUPLOIDY_CHECK."$TIMESTAMP".csv"

#######################################################################
##### Concatenate all per chromosome verifybamID reports together #####
#######################################################################

# ( cat $CORE_PATH/$PROJECT_MS/REPORTS/VERIFYBAMID_CHR/*.VERIFYBAMID.PER_CHR.txt | grep "^#" | uniq ; \
# cat $CORE_PATH/$PROJECT_MS/REPORTS/VERIFYBAMID_CHR/*.VERIFYBAMID.PER_CHR.txt | grep -v "^#" ) \
# | sed 's/\t/,/g' \
# >| $CORE_PATH/$PROJECT_MS/REPORTS/QC_REPORTS/$PROJECT_MS".PER_CHR_VERIFYBAMID."$TIMESTAMP".csv"
...
# {print $0}' \
# >|$CORE_PATH/$PROJECT_MS/REPORTS/$PROJECT_MS".WALL.CLOCK.TIMES.FIXED.csv"
