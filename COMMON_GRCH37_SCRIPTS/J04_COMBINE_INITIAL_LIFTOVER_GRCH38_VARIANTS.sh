...
# As is right now, I would imagine that this limits your scatter count...
	...
	# construct command line
		...
		for VCF in $(ls ${CORE_PATH}/${PROJECT_MS}/TEMP/${PREFIX}.*.FILTERED.GRCh38.LIFTOVER.vcf.gz)
		do
		  CMD=${CMD}" --variant ${VCF}"
		done

		CMD=${CMD}" --genotypemergeoption UNSORTED"
		CMD=${CMD}" -o ${CORE_PATH}/${PROJECT_MS}/MULTI_SAMPLE/${PREFIX}.FILTERED.GRCh38.LIFTOVER.vcf.gz"
