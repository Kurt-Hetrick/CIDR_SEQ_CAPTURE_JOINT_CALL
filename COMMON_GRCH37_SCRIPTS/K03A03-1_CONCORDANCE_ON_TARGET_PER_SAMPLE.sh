...
# intersect with the (fixed) target bed file.
	...
	zless ${FINAL_REPORT} \
		...
		| $BEDTOOLS_DIR/bedtools intersect \
			-a - \
			-b ${CORE_PATH}/${PROJECT_MS}/TEMP/${SM_TAG}/${SM_TAG}-${TARGET_BED_NAME}-LIFT_GRCH38_PRIMARY.bed \
	>| ${CORE_PATH}/${PROJECT_MS}/TEMP/${SM_TAG}/${SM_TAG}-${TARGET_BED_NAME}_FINAL_REPORT.GRCH38.bed
...
# using only those positions in the on target final report bed file.
	...
	CMD=${CMD}" SelectVariants"
		...
		CMD=${CMD}" --variant ${CORE_PATH}/${PROJECT_MS}/MULTI_SAMPLE/${PREFIX}.FILTERED.GT.REFINED.LIFTOVER.GRCH38.vcf.gz"
		CMD=${CMD}" --exclude-non-variants"
		CMD=${CMD}" --exclude-filtered"
		CMD=${CMD}" --remove-unused-alternates"
		CMD=${CMD}" --keep-original-ac"
		CMD=${CMD}" -L ${CORE_PATH}/${PROJECT_MS}/TEMP/${SM_TAG}/${SM_TAG}-${TARGET_BED_NAME}-LIFT_GRCH38_PRIMARY.bed"
		CMD=${CMD}" --sample-name ${SM_TAG}"
	CMD=${CMD}" --output ${CORE_PATH}/${PROJECT_MS}/TEMP/${SM_TAG}/${SM_TAG}_MS_FINAL_REPORT_ON_TARGET.GRCH38.vcf"
...
# Performs concordance between one vcf file and one final report. The vcf must be single sample.
	...
	# remove any record where the chromosome contains an underscore (typically alternate haplotypes, b/c cidrseqsuite can't handle them)
			...
			CMD="${JAVA_1_8}/java -jar"
				...
				# [1] path_to_vcf_file
				CMD=${CMD}" ${CORE_PATH}/${PROJECT_MS}/TEMP/${SM_TAG}/${SM_TAG}_MS_FINAL_REPORT_ON_TARGET.GRCH38.vcf"
				# [2] path_to_final_report_file
				CMD=${CMD}" ${FINAL_REPORT}"
				# [3] path_to_bed_file
				CMD=${CMD}" ${CORE_PATH}/${PROJECT_MS}/TEMP/${SM_TAG}/${SM_TAG}-${TARGET_BED_NAME}-LIFT_GRCH38_PRIMARY.bed"
