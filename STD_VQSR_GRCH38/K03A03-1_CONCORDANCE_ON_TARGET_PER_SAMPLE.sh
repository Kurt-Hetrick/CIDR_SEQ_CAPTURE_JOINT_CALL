...
# intersect with the (fixed) target bed file.
	...
	sed 's/\r//g' ${FINAL_REPORT} \
		...
		| $BEDTOOLS_DIR/bedtools intersect \
			-a - \
			-b ${CORE_PATH}/${PROJECT_MS}/TEMP/${SM_TAG}/${SM_TAG}-${TARGET_BED_NAME}.lift.hg19.bed \
	>| ${CORE_PATH}/${PROJECT_MS}/TEMP/${SM_TAG}/${SM_TAG}-${TARGET_BED_NAME}_FINAL_REPORT.hg19.bed
...
# using only those positions in the on target final report bed file.
	...
	CMD=${CMD}" SelectVariants"
		CMD=${CMD}" --reference ${HG19_REF}"
		CMD=${CMD}" --variant ${CORE_PATH}/${PROJECT_MS}/MULTI_SAMPLE/${PREFIX}.BEDsuperset.VQSR.1KG.ExAC3.REFINED.hg19.sorted.primary.vcf.gz"
		CMD=${CMD}" --exclude-non-variants"
		CMD=${CMD}" --exclude-filtered"
		CMD=${CMD}" --remove-unused-alternates"
		CMD=${CMD}" --keep-original-ac"
		CMD=${CMD}" -L ${CORE_PATH}/${PROJECT_MS}/TEMP/${SM_TAG}/${SM_TAG}-${TARGET_BED_NAME}_FINAL_REPORT.hg19.bed"
		CMD=${CMD}" --sample-name ${SM_TAG}"
	CMD=${CMD}" --output ${CORE_PATH}/${PROJECT_MS}/TEMP/${SM_TAG}/${SM_TAG}_MS_FINAL_REPORT_ON_TARGET.hg19.vcf"
