...
# Extract out sample, remove non-passing, non-variant
	...
	CMD=$CMD' --variant '$CORE_PATH'/'$PROJECT_MS'/MULTI_SAMPLE/'$PREFIX'.BEDsuperset.VQSR.SNP.HardFiltered.INDEL.1KG.ExAC3.REFINED.vcf'
	CMD=$CMD' -o '$CORE_PATH'/'$PROJECT_MS'/TEMP/'$SM_TAG'_SCATTER/'$SM_TAG'_MS_OnBait.'$CHROMOSOME'.vcf.gz'
...
echo $PROJECT_SAMPLE",K01,SELECT_ALL_SAMPLE_"$CHROMOSOME","$HOSTNAME","$START_SELECT_ALL_SAMPLE","$END_SELECT_ALL_SAMPLE \
